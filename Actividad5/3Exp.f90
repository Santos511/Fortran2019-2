...
IMPLICIT NONE
 ...
 REAL(kind=8),external:: ExpTaylor  !Esta es la variable que viene desde fuera
 REAL(kind=8)::y,x
 INTEGER:: n,i,a
    ...
    DO n=1,16,2
      a=a+1 !Cambiar color de la curva
       ...
       DO i=-500,500,1
         ...
         y=ExpTaylor(x,n) !Asigna a y la suma de Taylor de grado n y valor x
...
IMPLICIT NONE
 ...
 REAL(kind=8):: ExpTaylor    !Nombre con el que saldra el valor de la funcion
 ...
 INTEGER:: j
  ...
  suma=0 !Comenzamos dando valor 0 a la suma
   ...
   Taylor:DO j=0,n,1 
          a=x**j !Habiendo separado la sumatoria en tres partes queda como
          b=j     !a=numerador, b=denominador, de una misma operacion
           c=b         !Asignar el valor de b a d para calcular el factorial
           Factorial:DO    !Do para calcular el factorial de la siguiente forma:
                      IF(c==0) THEN  !b!=b*(b-1)*(b-2)*...*(b-n)*1
                         b=1         !En caso de que d sea 0 se le asigna el
                         EXIT        !valor de 1 a b y sale del DO
                      END IF   
                     c=c-1             
                     b=b*c
                      IF(c==1) EXIT 
           ...
           term=a/b     !Se realiza la operacion de la sumatoria
            suma=suma+term   !Se suma al valor anterior
   END DO Taylor

    ExpTaylor=suma !Se le asigna el valor de la sumatoria a la variable externa
